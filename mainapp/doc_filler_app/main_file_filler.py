...
class DomRF_Ipoteca:
    def write(client, doc):
        ...
        for i, value in Page.items():
            ...
            #Работа по совместительству (заполняется при наличии)
            #Иные доходы Заявителя
            #Дополнительная информация о занятости Заявителя (заполняется в случае применения опции «Легкая ипотека»)

            #Дополнительная информация о доходах от сдачи недвижимости в аренду (заполняется в случае применения опции
                #«Легкая ипотека» и при наличии доходов от сдачи недвижимости в аренду)
